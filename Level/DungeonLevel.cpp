...
void DungeonLevel::SetDungeonStage()
{
	...
	{
		GameInstance::GetInstance()->WriteLine(L"몬스터 데이터가 비어있습니다. 던전 레벨을 초기화할 수 없어 임의로 몬스터를 추가합니다.");
...
void DungeonLevel::Welcome()
{
	...
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"당신은 동굴 입구 앞에 섰습니다.");
	gameInstance->WriteLine(L"으스스한 동굴이 당신을 반깁니다..");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"도전하시겠습니까?");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"[던전 메뉴 옵션]");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"1. 인생은 모험이지! 도전한다.");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"2. 지금은 아닌것 같아.. 마을로 돌아갈래");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"============================================");
...
void DungeonLevel::ContinueExploration()
{
	...
	{
		gameInstance->WriteLine(L"");
		gameInstance->WriteLine(L"던전을 모두 탐험했습니다! ");
		gameInstance->WriteLine(L"당신은 던전의 모든 몬스터를 처치하고 돌아갑니다.");
		gameInstance->WriteLine(L"");
		gameInstance->WriteLine(L"1. 이제 충분해. 던전 입구로 돌아가자.");
		gameInstance->WriteLine(L"2. 이제 진짜 정말 충분해. 던전 입구로 돌아가자.");
		gameInstance->WriteLine(L"");
		gameInstance->WriteLine(L"============================================");
		gameInstance->WriteLine(L"원하는 옵션의 번호를 입력하세요.");
	...
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"당신은 던전을 더 탐험할 수 있습니다. 앞으로 나아갈까요?");
	wstring activeMonster = to_wstring(m_activeMonseters);
	gameInstance->WriteLine(L"남은 몬스터: " + activeMonster + L"마리");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"1. 계속해서 던전을 탐험한다.");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"2. 지금은 충분해. 던전 입구로 돌아간다.");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"============================================");
	gameInstance->WriteLine(L"원하는 옵션의 번호를 입력하세요.");
...
void DungeonLevel::OnEnterStage()
{
	...
	if (m_currentMonster == nullptr)
	{
		GameInstance::GetInstance()->WriteLine(L"몬스터를 생성할 수 없습니다. 초기화면으로 돌아갑니다..");
		Welcome();
		return;
	}
	...
	GameInstance* gameInstance = GameInstance::GetInstance();
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"던전에 입장합니다.");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"당신의 눈 앞에 " + m_currentMonster->GetName() + L"가(이) 등장했습니다!");
	gameInstance->WriteLine(L"상대는 아직 당신을 눈치채지 못했습니다.");
	gameInstance->WriteLine(L"전투를 시작할까요?");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"1. 지금이 기회다. 빠르게 적에게 달려간다.");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"2. 지금은 상태가 좋지않아. 던전 입구로 돌아간다.");
	gameInstance->WriteLine(L"");
	gameInstance->WriteLine(L"============================================");
	gameInstance->WriteLine(L"원하는 옵션의 번호를 입력하세요.");
...
void DungeonLevel::OnBackToVillage()
{
	...
	GameInstance::GetInstance()->WriteLine(L"때로는 한발 물러서는 것이 현명할 수 있습니다....");
	GameInstance::GetInstance()->WriteLine(L"당신은 발걸음을 돌려 마을로 돌아갑니다...");
...
void DungeonLevel::OnStartBattle()
{
	...
	{
		gameInstance->WriteLine(L"");
		gameInstance->WriteLine(L"당신은 전투에서 패배했습니다...");
		gameInstance->WriteLine(L"");
		gameInstance->WriteLine(L"1. 마을로 돌아가자...");
		gameInstance->WriteLine(L"2. 마을로...이제... 돌아가자...");
		gameInstance->WriteLine(L"");
		gameInstance->WriteLine(L"============================================");
		gameInstance->WriteLine(L"원하는 옵션의 번호를 입력하세요.");
...
{
	GameInstance::GetInstance()->WriteLine(L"");
	GameInstance::GetInstance()->WriteLine(L"당신은 빠르게 몸을 돌려 던전 입구로 뛰어갑니다.");

	if (rand() % 3 == 0)
	{
		GameInstance::GetInstance()->WriteLine(L"필사적으로 뛰었지만, "+ m_currentMonster->GetName() + L"가(이) 먼저 당신의 앞을 가로 막았습니다.");
		GameInstance::GetInstance()->WriteLine(m_currentMonster->GetName() + L"가(이) 당신을 공격합니다..");
		
		OnStartBattle();
	}
	else
	{
		GameInstance::GetInstance()->WriteLine(L"몬스터가 당신을 쫓아왔지만, 당신은 무사히 던전 입구로 도망쳤습니다.");
		Welcome();
	}
